...
public class Brain : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        neuralnet = new NNet();
        neuralnet.CreateNet(1, 5, 8, 2); //5 raycasts as inputs, 2 turning directions as outputs
        ...
        f = raycast.dis_f;
        fr = raycast.dis_fr;
        r = raycast.dis_fr; //Distances from a collision
    ...
    void Update()
    {
        hasFailed = hit.crash; //Whether the car has crashed or not
        ...
        if (!hasFailed)
        {
            dist += Time.deltaTime; //Increase the distance the car has travelled.
            List<float> inputs = new List<float>(); //List of distances from collisions
            ...
            neuralnet.SetInput(inputs); //Set these distances as inputs.
            neuralnet.refresh();

            leftForce = neuralnet.GetOutput(0); //Turn left and right are set as outputs
            rightForce = neuralnet.GetOutput(1);

            leftTheta = MAX_ROTATION * leftForce; //Apply rotations to these outputs
            rightTheta = MAX_ROTATION * rightForce;

            headingAngle += (leftTheta - rightTheta) * Time.deltaTime; //Apply rotations to car

            float speed = (Mathf.Abs(leftForce + rightForce)) / 2; //Setting speed values to include the turning force for more realistic turn speed.
            speed *= _SPEED;
        ...
        {
            dist = 0.0f; //If car has crashed, reset the distance.
        }
    }

    public float Normalise(float i) //Normalise the raycast distances to be between 0-1
    {
    ...
    public void ClearFailure() //If car has crashed, reset the values on the car for another genome to begin.
    {
        hasFailed = false;
        hit.crash = false;
    ...
    public float Clamp(float val, float min, float max) //Basic clamp function.
    {
        if (val < min)
        {
            return min;
...
public class NNet
{
    ...
    List<float> inputs = new List<float>(); //List of inputs for the neuralNet to use
    NLayer inputlayer = new NLayer();

    List<NLayer> hiddenLayers = new List<NLayer>(); //List of neuron layers.
    NLayer outputLayer = new NLayer();

    List<float> outputs = new List<float>(); //List of outputs for the car to use.
    ...
    public void refresh()
    {
        ...
        for (int i = 0; i < hiddenLayers.Count; i++)
        {
            if (i > 0)
            {
                inputs = outputs; //Set the inputs of the next layer to be the outputs of the previous layer
            }
    ...
    public void ReleaseNet() //Function to clear the NNet of all inputs, hidden and output layers.
    {
        if (inputlayer != null)
        {
            inputlayer = null;
    ...
    public Genome ToGenome()
    {
        ...
        for (int i = 0; i < this.hiddenLayers.Count; i++)
        {
            ...
            {
                genome.weights.Add(weights[j]); //Modify the weighting of each hidden layer.
            }
        }
        ...
        {
            genome.weights.Add(outweights[i]); //Modify the weighting of each output
        }
...
public class NLayer
{
    ...
    public float Sigmoid(float a, float p) //Hyperbolic Tangent activation function
    {
        float ap = (-a) / p;
    ...
    public void Evaluate(List<float> input, ref List<float> output)
    {
        ...
        for (int i = 0; i < totalNeurons; i++)
        {
            ...
            //Do the sizeof the weights - 1 to add in the bias to the activation afterwards.
    ...
    public void PopulateLayer(int numOfNeurons, int numOfInputs) //Give each neuron in the hidden layer an input to try.
    {
    ...
    public void SetWeights(List<float> weights, int numOfNeurons, int numOfInputs) //Set the weights of each Neuron in the hidden layer
    {
        int index = 0;
        totalInputs = numOfInputs;
        totalNeurons = numOfNeurons;
